...
# Unit cm/kg
box = Bin(
    partno='example0', width=589.8, height=243.8, depth=259.1, max_weight=28080, corner=15, bin_type=1
)
...
for i in range(5):
    packer.add_item(Item(
        partno='Cabinet{}'.format(str(i+1)), name='cabint', typeof='cube', width=60, height=80, depth=200, weight=80, level=1, loadbear=100, upside_down_=True, color='#842B00')
    )
...
# calculate packing
packer.pack(
    # Try switching fix_point=True/False to compare the results
    bigger_first=True, distribute_items=False, fix_point=False,
    check_stable=False, support_surface_ratio=0.75, number_of_decimals=0
)
